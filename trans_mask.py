...
def get_output(img_path, mask_rcnn, cartoon):  
    img = cv2.imread(img_path)
    img = inference.resize_crop(img)
    
    mask1,mask2 = inference.findmask_mask_rcnn(img, mask_rcnn, threshold=0.5)
...
if __name__=='__main__':
    ...
    # Load mask-rcnn and cartoonize GAN model
    ...
    for img in tqdm(imgs):
        output = get_output('input_image/'+img, mask_rcnn, cartoon)
